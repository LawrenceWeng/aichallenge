...
if ($compile_result) {
    while ($row = mysqli_fetch_assoc($compile_result)) {
        if (contest_query("update_submission_compiling",
            ...
            api_log(sprintf("Error updating submission for compile task: %s", mysqli_error($db_link)));
        }
    }
} else {
    api_log(sprintf("Error selecting next compile: %s", mysqli_error($db_link)));
}
...
if ($match_row) {
    ...
    if ($player_result) {
        ...
        api_log(sprintf("Error selecting matchup players: %s", mysqli_error($db_link)));
    }
} else {
    api_log(sprintf("Error selecting next match: %s", mysqli_error($db_link)));
}
