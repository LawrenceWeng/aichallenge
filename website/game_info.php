...
function get_error_message($game_id){
  ...
  $sql = "SELECT e.*, u.username from error e inner join submission s ON e.submission_id = s.submission_id inner join user u on u.user_id = s.user_id where game_id = '".mysqli_real_escape_string($db_link, $game_id)."'";
  $q = mysqli_query($db_link, $sql);
  $out = array();
