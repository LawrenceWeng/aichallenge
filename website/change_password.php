...
if (!logged_in_with_valid_credentials()) {
    ...
    if (isset($_GET['user_id']) && isset($_GET['code'])) {
        ...
        $user_id = mysqli_real_escape_string($db_link, stripslashes($_GET['user_id']));
        $forgot_code = mysqli_real_escape_string($db_link, stripslashes($_GET['code']));
        $naive_ip = mysqli_real_escape_string($db_link, $_SERVER['REMOTE_ADDR']);
        $real_ip = mysqli_real_escape_string($db_link, getRealIpAddr());
        
        $result = contest_query("log_login", $user_id, $naive_ip, $real_ip);
        if (!$result) {
          error_log("Could not write to log: " . mysqli_error($db_link));
        }
